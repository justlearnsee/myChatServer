...
int main(int argc, char** argv)
{
    if (argc < 3)
    {
        cerr << "command invalid! example: ./ChatClient 192.168.110.128 6000" << endl;
        exit(-1); 
    }
...
void doRegResponse(json& responsejs)
{
    ...
    {
        cout << "name register success, userid is " << responsejs["id"] << ", do not forget it!"
             << endl;
    }
}
...
void mainMenu(int clientfd)
{
    ...
    while (isMainMenuRunning)
    {
        ...
        it->second(clientfd,
                   commandbuf.substr(idx + 1, commandbuf.size() - idx));  // 调用命令处理方法
    }
}
...
// "creatgroup" command handler  groupname:groupdesc
